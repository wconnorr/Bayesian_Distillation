...
def calculate_actor_loss(actor, transition, epsilon):
  ...
  raw_policy = actor(states)
  current_policy = raw_policy[F.one_hot(actions.long(), raw_policy.size(1)).bool()]
...
def simple_act(policy, state):
  return torch.multinomial(torch.softmax(policy(state),1), 1).item()

def ensemble_act(models, state):
  return sp.stats.mode([simple_act(model, state) for model in models], keepdims=False)[0].item()

def perform_episode(policy, act_func, env, device):
  with torch.no_grad():
    sum_reward = 0
    state,_ = env.reset()
    done = False
    while not done:
      action = act_func(policy, torch.tensor(np.array(state)).float().to(device).unsqueeze(0))
